...
class QueryParam:
    ...
    mode: Literal["local", "global", "hybrid", "naive", "mix", "bypass","embedding","query_and_keywords","query_intersect_keywords"] = "global"
    ...
    - "embedding": 这个是我加的，就是直接基于original query去检索KG，可以采用优先检索实体然后再顺带查出与之相关的关系和文本块的方法，也可以采用先检索关系然后再检索出与之相关的实体的方法.
    - "query_and_keywords": 这个是我加的，结合了关键词检索和基于原始查询的检索，先分别执行关键词和原始查询的节点和边检索，然后合并结果，优先考虑原始查询的结果。
    - "query_intersect_keywords": 这个是我加的，结合了关键词检索和基于原始查询的检索，先分别执行关键词和原始查询的节点和边检索，然后取交集结果，优先考虑原始查询的结果。
    """
