...
class LightRAG:
    ...
    # 🔥 新增：检索日志配置
    enable_retrieval_logging: bool = field(default=False)
    """If True, enables logging of retrieval results for evaluation purposes."""

    retrieval_log_dir: str | None = field(default=None)
    """Directory for storing retrieval logs. If None, defaults to {working_dir}/retrieval_logs."""
    # Storages Management
    # ---
    ...
    def __post_init__(self):
        ...
        # 🔥 新增：初始化检索日志记录器
        self.retrieval_logger: RetrievalLogger | None = None
        if self.enable_retrieval_logging:
            log_dir = self.retrieval_log_dir or os.path.join(
                self.working_dir, "retrieval_logs"
            )
            self.retrieval_logger = RetrievalLogger(log_dir=log_dir)
            logger.info(f"Retrieval logging enabled. Logs will be saved to: {log_dir}")
        else:
            logger.info("Retrieval logging is disabled")
    ...
    async def ainsert_custom_kg(
        ...
        try:
            ...
            if len(all_relationships_data) > 0:
                sample_dp = all_relationships_data[0]
                sample_content = f"{sample_dp['src_id']}\t{sample_dp['tgt_id']}\n{sample_dp['description']}"
                print(f"👀 [VERIFY] Relationship Vector Content Sample (Custom KG): {repr(sample_content)}")
    ...
    # 🔥 在这里添加这两个方法（与 query 方法同级缩进）
    def get_retrieval_statistics(self, log_file: str | None = None) -> dict[str, Any]:
        """
        Get retrieval statistics from logs.
        
        Args:
            log_file: Specific log file path. If None, uses current session log.
        
        Returns:
            Dictionary containing retrieval statistics.
        """
        if not self.retrieval_logger:
            logger.warning("Retrieval logging is not enabled")
            return {}
        
        return self.retrieval_logger.get_statistics(log_file)
    
    def export_retrieval_logs(self, output_file: str, log_file: str | None = None) -> None:
        """
        Export retrieval logs to a JSON file.
        
        Args:
            output_file: Path to save the exported JSON file.
            log_file: Source log file. If None, uses current session log.
        """
        if not self.retrieval_logger:
            logger.warning("Retrieval logging is not enabled")
            return
        
        self.retrieval_logger.export_to_json(output_file, log_file)
        logger.info(f"✅ Retrieval logs exported to: {output_file}")
    ...
    async def aquery(
        ...
        if param.mode in ["local", "global", "hybrid", "mix","embedding","query_and_keywords","query_intersect_keywords"]:
            # 🔥 修改：传递 retrieval_logger
            response = await kg_query(
                ...
                retrieval_logger=self.retrieval_logger,  # 🔥 新增参数
            )
