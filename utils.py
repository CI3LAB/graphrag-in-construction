...
def process_intersection_contexts(list_a: list[dict], list_b: list[dict]) -> list[dict]:
    """
    Calculate the intersection of two context lists.
    Only content present in both list_a and list_b will be kept.

    Args:
        list_a: First context list
        list_b: Second context list

    Returns:
        Intersection context list
    """
    if not list_a or not list_b:
        return []

    # 1. Convert list_a content to a hashable set for fast lookup
    set_a = set()
    for item in list_a:
        # Exclude id field, compare content only
        content_dict = {k: v for k, v in item.items() if k != "id"}
        # Convert dict to sorted tuple to make it hashable
        content_key = tuple(sorted(content_dict.items()))
        set_a.add(content_key)

    intersection_data = []
    seen_in_result = set()

    # 2. Iterate through list_b, check if exists in set_a
    for item in list_b:
        content_dict = {k: v for k, v in item.items() if k != "id"}
        content_key = tuple(sorted(content_dict.items()))

        if content_key in set_a and content_key not in seen_in_result:
            intersection_data.append(item)
            seen_in_result.add(content_key)

    # 3. Reassign IDs
    for i, item in enumerate(intersection_data):
        item["id"] = str(i + 1)
